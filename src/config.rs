...
impl Config {
    ...
    fn validate(&self) -> Result<()> {
        // An empty path used to silently mean "disabled", which made a typo'd
        // deployment indistinguishable from a deliberate opt-out.
        if self.metrics.stats_db_path.as_deref() == Some("") {
            anyhow::bail!(
                "[metrics] stats_db_path is empty: set a path or remove the key \
...
/// The sections of [`Config`], for validating override targets. Must move with
/// the struct's fields; `config_sections_match_the_struct` pins the two
/// together.
...
fn apply_env_overrides(
    ...
    for (name, raw) in vars {
        ...
        // The section list is closed, so check it here where the error can
        // name the offending variable. A typo'd key inside a real section is
        // caught downstream by serde's deny_unknown_fields; the override line
        // printed below says which variable minted it.
        if !CONFIG_SECTIONS.contains(&section.as_str()) {
            anyhow::bail!(
                "{name}: '{section}' is not a config section (one of {})",
...
"#
    ...
    /// The shipped example must parse under `deny_unknown_fields`, or the
    /// first thing a fresh deployment sees is a boot failure.
    #[test]
    ...
    /// An unknown key anywhere in the file fails the load with serde naming
    /// the field — a stale key from a removed feature and a typo'd new one are
    /// the same mistake, and both used to be silently ignored.
    #[test]
    fn unknown_config_keys_are_rejected() {
        ...
        let src = base.replace(
            "idle_timeout_secs = 300",
            "idle_timeout_secs = 300\ncoinbase_address = \"bc1qold\"",
        );
        let err = toml::from_str::<Config>(&src).unwrap_err().to_string();
        assert!(err.contains("coinbase_address"), "unexpected error: {err}");

        let src = format!("{base}\n[typo_section]\nkey = 1\n");
        assert!(toml::from_str::<Config>(&src).is_err());
    }

    /// Every real section accepts an override; an invented one errors naming
    /// the variable, instead of minting a table serde then rejects with a
    /// message that never mentions the environment.
    #[test]
    fn config_sections_match_the_struct() {
        let base = config_toml("/btcpool-rs/", 4, 4);
        for section in super::CONFIG_SECTIONS {
            let var = format!("BTCPOOL_{}__PROBE", section.to_uppercase());
            let v = apply(&base, &[(&var, "\"x\"")]).unwrap();
            assert!(
                v[*section].get("probe").is_some(),
                "override into [{section}] was not applied"
            );
        }

        let err = apply(&base, &[("BTCPOOL_TYPO_SECTION__KEY", "1")])
            .unwrap_err()
            .to_string();
        assert!(
            err.contains("BTCPOOL_TYPO_SECTION__KEY") && err.contains("not a config section"),
            "unexpected error: {err}"
        );
    }
